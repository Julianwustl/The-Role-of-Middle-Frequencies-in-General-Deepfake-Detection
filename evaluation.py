...
from core.image import load_and_preprocess_image, read_and_resize_image
from loader.transforms import BandFilter, FDAFilter
from callbacks import EmbeddingCollectorCallback, ConfusionMatrixCallback
...
def run_evaluation(args, filter=None, radius=None):
    ...
    trainer = Trainer(
        ...
        #             args.model_checkpoint.split("/")[-1].split("_")[1],
        #         ),
        #         file_name=f"{args.encoder_type}_{args.filter_type}_{args.data_type}_{radius}",
        #     ),
        # ],
    )
    start_time = time.time()
