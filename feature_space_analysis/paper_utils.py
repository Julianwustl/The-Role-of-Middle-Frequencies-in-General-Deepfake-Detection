...
def create_subplot_mosaic(total_plots, max_per_row=4, **kwargs):
    """
    Dynamically create a grid of subplots using subplot_mosaic based on the total number of plots.
    Organizes the plots with a maximum number of `max_per_row` plots per row.

    Parameters:
    - total_plots (int): Total number of plots/subplots desired.
    - max_per_row (int): Maximum number of plots per row.

    Returns:
    - fig (matplotlib.figure.Figure): Figure object.
    - axd (dict): Dictionary of Axes objects.
    """
    import matplotlib.pyplot as plt

    # Calculate the number of rows required
    num_rows = -(-total_plots // max_per_row)  # Ceiling division
    ...
    for i in range(num_rows):
        for j in range(max_per_row):
            if plot_counter < total_plots:
                layout_str += str(plot_counter)
                plot_counter += 1
            else:
                layout_str += "."
        layout_str += "\n"

    # Generate height_ratios and width_ratios
    height_ratios = [1] * num_rows
    width_ratios = [1] * max_per_row

    fig, axd = plt.subplot_mosaic(
        layout_str, height_ratios=height_ratios, width_ratios=width_ratios, **kwargs
    )
...
def configure_matplotlib(
    ...
    num_colors = len(sns.color_palette("deep"))

    # Generate dashes for each color
    # This is just a simple example; you can customize it further
    dashes = [(i, i) for i in np.linspace(2, 5, num_colors)]
    dashes[0] = "-"
